...
import pl.rmakowiecki.smartalarm.feature.main.alarmincidents.AlarmIncidentsViewStateChange.*
...
class AlarmIncidentsViewStateReducer @Inject constructor() {
    ...
    fun reduce(currentViewState: AlarmIncidentsViewState, change: AlarmIncidentsViewStateChange): AlarmIncidentsViewState = when (change) {
        ...
        is ItemArchived -> currentViewState
        is ItemDeleted -> currentViewState
        is ItemsEmpty -> {
            if (change.isEmpty) {
                currentViewState.copy(
                        isLoading = false,
                        isPlaceholderVisible = true,
                        incidentsList = emptyList()
                )
            } else currentViewState
        }
        is ItemAdded -> currentViewState.copy(
                isLoading = false,
        ...
        is ItemUpdated -> currentViewState.copy(
                isLoading = false,
                incidentsList = currentViewState.incidentsList.toMutableList().apply {
        ...
        is SnackBarShown -> currentViewState.copy(
                isSnackBarShown = true,
                snackBarMessage = change.message
        )
        is SnackBarHidden -> currentViewState.copy(
